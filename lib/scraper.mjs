...
// Firefox 120 ClientHello spec
export const FIREFOX_120_SPEC = {
  cipherSuites: [0x1301, 0x1303, 0x1302, 0xc02b, 0xc02f, 0xcca9, 0xcca8, 0xc02c, 0xc030, 0xcca9, 0xc00a, 0xc009, 0xc013, 0xc014, 0x009c, 0x009d, 0x002f, 0x0035, 0x000a],
  extensions: [
    { type: 0x0000 }, { type: 0x000d }, { type: 0x000a }, { type: 0x002b },
    { type: 0x0029 }, { type: 0x0017 }, { type: 0xff01 }, { type: 0x0023 },
    { type: 0x0010, alpnProtocols: ['h2', 'http/1.1'] }, { type: 0x0005 },
    { type: 0x0033 }, { type: 0x002d }, { type: 0x001c },
  ],
  supportedGroups: [0x001d, 0x0017, 0x0018],
  signatureAlgorithms: [0x0403, 0x0503, 0x0603, 0x0804, 0x0805, 0x0806, 0x0401, 0x0501, 0x0601, 0x0201],
  alpnProtocols: ['h2', 'http/1.1'],
};

// Safari 17 ClientHello spec
export const SAFARI_17_SPEC = {
  cipherSuites: [0x1301, 0x1302, 0x1303, 0xc02c, 0xc02b, 0xcca9, 0xcca8, 0xc030, 0xc02f, 0xc013, 0xc014, 0x009c, 0x009d, 0x002f, 0x0035],
  extensions: [
    { type: 0x0000 }, { type: 0x0017 }, { type: 0x000d }, { type: 0x000a },
    { type: 0x002b }, { type: 0xff01 }, { type: 0x0023 },
    { type: 0x0010, alpnProtocols: ['h2', 'http/1.1'] }, { type: 0x0033 },
    { type: 0x002d }, { type: 0x001c }, { type: 0x0015 },
  ],
  supportedGroups: [0x001d, 0x0017, 0x0018],
  signatureAlgorithms: [0x0403, 0x0804, 0x0401, 0x0503, 0x0501, 0x0803, 0x0601, 0x0201],
  alpnProtocols: ['h2', 'http/1.1'],
...
export const TLS_PROFILES = {
  chrome120: CHROME_120_SPEC,
  firefox: FIREFOX_120_SPEC,
  safari: SAFARI_17_SPEC,
};

export function resolveTlsProfile(job) {
  // If job.tls_profile is a string, look up preset
  if (typeof job.tls_profile === 'string') {
    const preset = TLS_PROFILES[job.tls_profile];
    if (!preset) {
      throw new Error(`unknown tls_profile: ${job.tls_profile}. Available: ${Object.keys(TLS_PROFILES).join(', ')}, or pass a custom spec object`);
    }
    return preset;
  }
  // If job.tls_profile is an object, use it as a custom spec
  if (typeof job.tls_profile === 'object' && job.tls_profile !== null) {
    // Validate it has the required fields
    if (!Array.isArray(job.tls_profile.cipherSuites)) {
      throw new Error('custom tls_profile must have cipherSuites array');
    }
    if (!Array.isArray(job.tls_profile.extensions)) {
      throw new Error('custom tls_profile must have extensions array');
    }
    return job.tls_profile;
  }
  // Default to Chrome 120
  return CHROME_120_SPEC;
}
...
export async function fetchWithTlsImpersonate(job, timeoutMs, tlsImpersonateModule) {
  ...
  const spec = resolveTlsProfile(job);
